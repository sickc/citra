...
void RO::LoadCRO(Kernel::HLERequestContext& ctx, bool link_on_load_bug_fix) {
    ...
    CROHelper cro(cro_address, *process, system.Memory(), system.GetRunningCore());
    ...
    system.GetRunningCore().InvalidateCacheRange(cro_address, cro_size);
...
void RO::UnloadCRO(Kernel::HLERequestContext& ctx) {
    ...
    system.GetRunningCore().InvalidateCacheRange(cro_address, fixed_size);
...
void RO::Shutdown(Kernel::HLERequestContext& ctx) {
    ...
    CROHelper crs(slot->loaded_crs, *process, system.Memory(), system.GetRunningCore());
    crs.Unrebase(true);

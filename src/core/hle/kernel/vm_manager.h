...
public:
     ...
     */
    ResultVal<VAddr> MapBackingMemoryToBase(VAddr base, u32 region_size, Memory::MemoryRef memory, u32 size,
                                            MemoryState state);
    /**
    ...
    ResultVal<std::vector<std::pair<Memory::MemoryRef, u32>>> GetBackingBlocksForRange(VAddr address,
                                                                               u32 size);
